...
function AppChat(props) {
  ...
  //Implementation of earlier conversations
    ...
    useEffect(() => {
        ...
        axios.get(`${BACKEND_URL}/api/conversations`, {
            params: {
                conversation_id: props.auth["email"]
            },
            headers:{
  ...
  const handleSubmit = (e) => {
    ...
    async function callAPI() {
      console.log('sender email: '+ props.auth["email"])
      try {
        const response = await fetch("http://137.63.194.17:30010/webhooks/rest/webhook/", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ sender: props.auth["email"], message: inputPrompt }),
        });
    ...
    <div className="App">
      ...
      <aside className="sideMenu">
        ...
        />
         ...
         <div className="versionx">
         <div className="vr">
            <img src={props.auth ? props.auth["picture"]:""} alt="Profile picture" />
        </div>

        <div className="vx" >
         {props.auth ? props.auth["name"]:""} 
        </div>
      ...
      <img src={props.auth ? props.auth["picture"]:""} alt="Profile picture" />
      <h1>Hello {props.auth ? props.auth["name"]:""}, Welcome to COVID CHATBOT PLATFORM</h1>
      <h2>Email: {props.auth ? props.auth["email"]:""}</h2>
      {/*<button onClick={()=>handleLogout()}>Logout</button> 
  </div>
      </section> */}
    </div>
